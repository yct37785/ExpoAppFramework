...
const LayoutScreen: Screen.ScreenType = ({ navigation, route }) => {
  ...
  const BasicContainer: React.FC<{ i: number, width: number, height: number, backgroundColor?: string }>
    = ({ i, width, height, backgroundColor = tier2 }) => (
      <View style={{ width: width, height: height, alignItems: 'center', justifyContent: 'center', backgroundColor }}>
      ...
      <UI.VerticalLayout constraint='scroll'>
        ...
        <UI.HorizontalLayout constraint='none' backgroundColor={tier1}>
          ...
          <UI.VerticalLayout constraint='wrap' justify='center' flex={1} backgroundColor={tier2}>
            <BasicContainer i={1} width={small} height={small} backgroundColor={tier3} />
          </UI.VerticalLayout>
